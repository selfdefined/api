...
The database structure displayed above can be broken down into 4 grouping which are displayed in columns from left to right.

They are:

1. Words table to which all other data is related. Each row here is equivalent to a markdown file in the [definitions folder](https://github.com/selfdefined/web-app/tree/prod/11ty/definitions).
...
4. The fourth column of tables are lists that provide additional data that can be shared by the items in their table in the third column
...
4. The third column of tables can be directly accessed by their unique ID or unique slug. Then through the second column of tables words can be found that related to those items.
...
These master lists can be added to in whatever method we determine necessary. Also, each of these tables relationships to a word is optional. We can make them required on the front end if desired.
